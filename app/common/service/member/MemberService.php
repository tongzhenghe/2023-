...
class MemberService
{
    ...
    public static function list($where = [], $page = 1, $limit = 10, $order = [], $field = '')
    {
        ...
        if (empty($field)) {
            $field = $group . ',headimgurl,add_id,avatar_id,nickname,username,phone,email,sort,is_super,is_disable,create_time, ide_id, is_member,QQ, wx, openid';
        }
        if (empty($order)) {
            $order = [$group => 'desc'];
        }
        ...
        $list = $model->field($field)->where($where)
            ->with(['avatar', 'tags', 'groups', 'ide', 'address'])
